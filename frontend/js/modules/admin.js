    ...
    var init = function (resp) {
      ...
      m.request({
        ...
        form.initFields(c, ['title', 'rootUrl', 'allowEtherPads',
          ...
          'openRegistration', 'hideHelpBlocks', 'useDisplayNameInPads']);
  ...
  */
  ...
  view.settings = function (c) {
    ...
    var f = {
      ...
      useDisplayNameInPads: (function () {
        var icon = form.icon(c, 'useDisplayNameInPads', A.INFO.USE_DISPLAY_NAME_IN_PADS);
        var opts = {
          name: 'useDisplayNameInPads',
        };
        if (c.data.useDisplayNameInPads) {
          opts['checked'] = 'checked'
        }
        var f = m('input[type="checkbox"]', opts);
        ld.assign(f.attrs, {
          type: 'checkbox',
          checked: c.data.useDisplayNameInPads(),
          onchange: m.withAttr('checked', c.data.useDisplayNameInPads)
        });
        var l = m('label', [ f, A.FIELD.USE_DISPLAY_NAME_IN_PADS, icon ]);
        return l;
      })(),
    ...
    return m('form', {
      ...
      m('fieldset', [
        ...
        m('div', [
          ...
          m('div.checkbox',   [ f.useDisplayNameInPads ]),
