...
var JOBQ_PREFIX = require('../storage').DBPREFIX.JOBQ;
...
function deletePads(nextLoop) {
  ...
  db.findKeys(JOBQ_PREFIX+'deletePad:*', null, function(err, pads) {
    if (err) { return exitIfErr(err); }
    ...
    if (pads !== null && pads.length > 0) {
      ...
      eachOfSeries(pads, function(padId, index, next) {
        padId = padId.substring(26); // remove "mypads:jobqueue:deletePad:"
